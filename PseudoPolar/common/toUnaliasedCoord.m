...
for k=1:length(aCoord)
    if scalar
        %old: uCoord{k} = toUnaliasedIdx(aCoord(k),N);        
        uCoord{k} = aCoord(k)+floor(N/2)+1;

    else
        %old: uCoord{k} = toUnaliasedIdx(aCoord(k),N(k));
        uCoord{k} = aCoord(k)+floor(N(k)/2)+1;

    end
end

%Revision Record
%  19/03/07 Yoel Shkolnisky    Optimization. Replace call to toUnaliasedIdx
%                              with in-place evaluation.
