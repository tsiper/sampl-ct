...
for k=-n:n
   ...
   v = cfrft(u,-k*alpha);
   tmp1(toUnaliasedIdx(k,2*n+1),:) = v(1:n);
end

tmp1 = m*icfftd(tmp1,1);
adjpp1 = flipud(tmp1(n/2+1:3*n/2,:));
...
for k=-n:n
   ...
   v = cfrft(u,-k*alpha);
   tmp2(toUnaliasedIdx(k,2*n+1),:) = v(1:n);
end
...
tmp2 = m*icfftd(tmp2,1);
tmp2 = tmp2.';
adjpp2 = flipud(tmp2(:,n/2+1:3*n/2,:));
