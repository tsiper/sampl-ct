...
%  inputs:
%    chem_spec: (string) chemical formula for the compound e.g. NaSO4 or H2O
...
nreqargs = 2;
if(nargin<nreqargs), error('%d args required, only have %d',nreqargs,nargin);end
...
for kz = 1:numel(atomic_numbers)
  ...
  a = interp1(etable, mus, log(egys), 'pchip'); % piecewise cubic Hermite polynomial
