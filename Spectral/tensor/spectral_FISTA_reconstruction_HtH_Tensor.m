function [ a_hat,f_vals,psnr_vals,w_best] = spectral_FISTA_reconstruction_HtH_Tensor( p_input,h,h_grad,a_input,...
...
for k =1:ReconstructionParams.iters
    ...
    for m = 1:I
        ...
        %         temp_w = conjgrad(@(x) Rpp_T(M(Rpp(x))) , Rpp_T(M(arg_k{m})),zeros(Y),50,1e-60);
        ...
        norm_x_k = FGP( norm_w_k , ReconstructionParams.lambda/sqrt(k),...
            ReconstructionParams.TV_Iters);%
        w_k{m} = norm_x_k*range_w+min_w;
        
        %%
        % norm_x_k = Hpp(FISTA_TV(HtH,b, ...))
        %
        % z_k{m} = Rpp(w_k{m});
        ...
        %         x_tv = FISTA_TV_Spectral(b,AtA,zeros(Y),L_fista,ReconstructionParams.lambda,FISTA_Iters);
        %         w_k{m} = double(x_hat);
