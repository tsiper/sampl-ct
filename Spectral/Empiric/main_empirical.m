...
SpectralParams.Vtube = [70 ,120];
% The bin right limits markers, the first bin starts at 0
SpectralParams.Threshold = [70 ,120];
...
% % FIXME: A normalizing vector, trying to equalize energies between materials
% base = [1.052,1.145];
ChosenMaterials = {'C', 'water'}; %choose the materials you wish for the basis
% FIXME: A normalizing vector, trying to equalize energies between materials
base = [1,1];
...
load('spectras.mat');
SpectralParams.extSpectra = cell(K,1);
for n = 1:K

            spectrum.energies = 
            y_temp = spectrum.specnum;
spectrum = Params.extSpectra{n}
			SpectralParams.extSpectra{n}.
end
Spectra = GetSpectra('ExtSpectra', SpectralParams);
...
L =0.5*SpectralParams.K; %Lipschitz coeeficient. optimal 90 with calibration region , 800 for distinct materials,0.5adipose&water
