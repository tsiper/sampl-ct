...
% x_out= FISTA_TV( All_LR_col,A,At,x_start,1e-3,20,1000,20,y_radon_original);
% [ x_out, f_vals ] = MFISTA( All_LR_col,A,At,x_start,1e-3,20,300,20,y_radon_original);
At_b = All_W'*All_LR_col;
AtA = @(x) All_W'*All_W*x;
% [ x_out, f_vals ] = FISTA_TV_PP( At_b,AtA,x_start,20,300,y_radon_original);
x_out_recon=reshape(x_out,size(y_radon_original));
%% check PSNR
 
PSNR_sinogram = psnr(x_out_recon , y_radon_original);
figure(6)
ax(2)=subplot(121);imagesc(y_radon_original);
ax(1)=subplot(122);imagesc(x_out_recon);
linkaxes(ax);
