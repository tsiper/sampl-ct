function [ x_star, f_vals ] = FISTA_TV_Spectral( At_b,AtA,x_start,L,lambda,iters,x_true)
...
%     x_k = FGP_unconstrained( y_k - (1/L) * ( Diff ) , lambda/L, TV_Iters);
     y_temp = y_k - (1/L) * ( Diff );
    min_y = min(y_temp(:));
    range_y = range(y_temp(:));
    norm_y_k = (y_temp-min_y)/range_y;
    norm_x_k = FGP( norm_y_k , lambda/L, TV_Iters);%
    x_k = norm_x_k*range_y+min_y;
