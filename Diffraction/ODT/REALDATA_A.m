...
loc= 'D:\Project - OCT - Ariel and Tal\Gilis functions\';
%%
%Read
load('COM.mat') %From FIRSTRUN 
col_cm = x_c; 
row_cm = y_c; 
I= zeros(SIZE, SIZE,  74);
temp = rgb2gray(imread(strcat(loc,'T_cell\T_xy_1.png')));
I(:,:,1)= temp(row_cm(1)-SIZE/2 :row_cm(1)+ SIZE/2-1 ,col_cm(1)-SIZE/2 :col_cm(1)+ SIZE/2-1); %First cut

for a= 1:73
temp = rgb2gray(imread(strcat(loc,'T_cell\', num2str(a),'.png')));
...
% n_radon= tomo_radon_2Ds(phase_true ,lambda, pix_CCD, Mag, teta, nm );
