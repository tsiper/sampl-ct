function [OutputImage,ReconstructedPhantomSamples] = Run_SPURS_Diffraction(Data,SamplingGrid)
...
% sqrtN = numel(SamplingGrid); % points per row / column of the cartesian grid

% OVERRIDE
sqrtN = 256;
SamplingGridCoordinates = SamplingGrid;
...
%==>SPURS Parameters
BsplineDegree = 3;
OverGridFactor = 1; % AT - was 2
Rho = 1e-5;
Niterations = 3; % AT - was 5
...
% axis([-1 1 -1 1].*sqrtN); axis square;
% hold on;
...
SPURS_settings.ForceGenrateNewPhi = 0; % AT - was 0
SPURS_settings.ForceFactorPsi = 1; % AT - was 1
...
%b = reshape(flipud(Data),numel(Data),1); % AT - Fitting Data matrix to needed vector foramt
b=Data;


% tic
[OutputImages, ReconstructedPhantomSamples] = SPURS(b, SamplingGridCoordinates, SPURS_settings);
OutputImage = OutputImages(:,:,Niterations);
